...
from ..nlspnmodel import NLSPN
from ..common import conv_bn_relu

@HEADS.register_module()
class DDIMDepthRefine2_NLSPNBlur(BaseDepthRefine):
    ...
    def __init__(
        ...
        self.ddiml1rec = L1Loss(self.init_cfg)
        self.ddiml2rec = L2Loss(self.init_cfg)
        ...
        self.guidance_projection = conv_bn_relu(channels_in+kwargs['depth_feature_dim'], self.num_neighbors, kernel=3, stride=1,
                                    padding=1, bn=False, relu=False)
        if self.init_cfg.conf_prop:
            # Confidence Branch
            # Confidence is shared for propagation and mask generation
            # 1/1
            self.cf_dec1 = conv_bn_relu(channels_in, 32, kernel=3, stride=1,
                                        padding=1)
            self.cf_dec0 = nn.Sequential(
                nn.Conv2d(32+kwargs['depth_feature_dim'], 1, kernel_size=3, stride=1, padding=1),
                nn.Sigmoid()
            )
         
        self.refine_layer = NLSPN(self.init_cfg, self.num_neighbors, 1, 3,
                                self.init_cfg.prop_kernel)
    ...
    def forward(self, fp, depth_map, depth_mask, gt_depth_map=None, return_loss=False, **kwargs):
        ...
        guidance = self.guidance_projection(torch.cat((x, sparse_depth_t), dim=1))

        if self.init_cfg.conf_prop:
            # Confidence Decoding
            cf_fd1 = self.cf_dec1(x)
        ...
        ddiml1 = self.ddiml1rec(refined_depth, gt_depth_map)
        ddiml2 = self.ddiml2rec(refined_depth, gt_depth_map)
        
        guidance = F.interpolate(guidance, scale_factor=2)
        confidence = F.interpolate(confidence, scale_factor=2)
        y, y_inter, offset, aff, aff_const = \
            self.refine_layer(refined_depth, guidance, confidence, sparse_depth, rgb=None)
        
        # prop_refined_depth_t = self.depth_transform.t(y)
        ...
        ddim_total = ddim_loss + ddiml1 + ddiml2

        output = {'pred': y, 'pred_init': depth_map_t, 'blur_depth_t': depth_map_t , 'refineddepth': refined_depth,
                'ddim_loss': ddim_total, 'gt_map_t': gt_map_t, 
                'pred_uncertainty': None,
                 'pred_inter': y_inter, 'weight_map': None,
