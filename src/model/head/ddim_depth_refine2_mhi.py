...
class DDIMDepthRefine2_MHI(BaseDepthRefine):
...
class ScheduledCNNRefine(BaseModule):
    def __init__(self, channels_in, channels_noise, **kwargs):
        ...
        self.mid_fusion = nn.Sequential(
            nn.Conv2d(channels_in+channels_noise, 64, kernel_size=3, stride=1, padding=1),
            nn.GroupNorm(4, 64),
            # 不能用batch norm，会统计输入方差，方差会不停的变
            nn.ReLU(True),
            nn.Conv2d(64, channels_in, kernel_size=3, stride=1, padding=1),
        ...
        self.feature_downsample = Downsample(channels_in)
    ...
    def forward(self, noisy_image, t, *args):
        ...
        feat_down = self.feature_downsample(feat, t)
        sparse_depth_down = self.depth_downsample(sparse_depth, t)
        midfusion = torch.cat((feat_down, sparse_depth_down),dim=1)
        midfusion = self.mid_fusion(midfusion)
