...
class DDIMDepthRefine2_Self_Guide(BaseDepthRefine):
...
class Project_Decoder(nn.Sequential):
    ...
    def __init__(self, skip_input, output_features, conv_cfg=None, norm_cfg=None, act_cfg=None):
        super(Project_Decoder, self).__init__()
    ...
    def forward(self, x, concat_with, time_embedding):
        up_x = F.interpolate(x, size=[concat_with.size(2), concat_with.size(3)], mode='bilinear', align_corners=True)
        return self.convB(self.convA(torch.cat([up_x, concat_with+time_embedding], dim=1)))
...
class ScheduledCNNRefine(BaseModule):
    def __init__(self, channels_in, channels_noise, **kwargs):
        ...
        self.time_embedding_2 = nn.Embedding(1280, channels_noise)
        self.guidance_layer = Project_Decoder(channels_in + channels_noise, channels_in)
    ...
    def forward(self, noisy_image, t, *args):
        ...
        merged_ret = self.guidance_layer(feat, sparse_depth, self.time_embedding_2(t)[..., None, None])
        # 这里考虑加个noise
        ret = self.pred(merged_ret)
