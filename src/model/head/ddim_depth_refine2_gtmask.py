...
class DDIMDepthRefine2_GTMask(BaseDepthRefine):
    ...
    def ddim_loss(self, gt_depth, refine_module_inputs, blur_depth_t, weight, **kwargs):
        ...
        mask = (gt_depth > t_valid).type_as(noise).detach()
        ...
        d = torch.pow(noise_pred - noise, 2) * mask
    ...
    def ddim_loss_gt(self, gt_depth, refine_module_inputs, blur_depth_t, weight, **kwargs):
        # Sample noise to add to the images
        t_valid = 0.0001
        # noise = torch.randn(gt_depth.shape[0], 16, int(gt_depth.shape[2]/2), gt_depth.shape[3]/2).to(gt_depth.device)
        bs = gt_depth.shape[0]
        mask = (gt_depth > t_valid).type_as(gt_depth).detach()
        # print(mask.shape)
        mask = F.max_pool2d(mask, kernel_size=3, stride=2, padding=1)
        # print(mask.shape)
        noise = torch.randn(blur_depth_t.shape).to(gt_depth.device)
