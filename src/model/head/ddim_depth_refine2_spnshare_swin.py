...
class DDIMDepthRefine2_SPNShare_Swin(BaseDepthRefine):
    ...
    def __init__(
        ...
        channels_in = in_channels[0] + self.depth_embed_dim
        ...
        depth_feature_dim = kwargs['depth_feature_dim']
...
class SPNDDIMPipiline(BaseModule):
    ...
    def _propagate_once(self, feat, offset, aff):
        depth_map_num = feat.shape[1]
        repeat_w = torch.repeat_interleave(self.w, repeats=depth_map_num, dim=0)
        repeat_b = torch.repeat_interleave(self.b, repeats=depth_map_num, dim=0)
        # group = depth_map_num 
        feat = ModulatedDeformConvFunction.apply(
            feat, offset, aff, repeat_w, repeat_b, self.stride, self.padding,
            self.dilation, depth_map_num, self.deformable_groups, self.im2col_step
        )
    ...
    def forward(
        ...
        feat = F.interpolate(feat, size=[sparse_depth_t.size(2), sparse_depth_t.size(3)], mode='bilinear', align_corners=True)
