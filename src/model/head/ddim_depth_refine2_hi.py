...
class DDIMDepthRefine2_HI(BaseDepthRefine):
...
class ScheduledCNNRefine(BaseModule):
    def __init__(self, channels_in, channels_noise, **kwargs):
        ...
        self.downblock = DownBlock(in_channels=channels_in*2, out_channels=channels_in, time_channels=1280, has_attn=True)
        self.upblock = UpBlock(in_channels=channels_in+channels_in+channels_noise, out_channels=channels_in, time_channels=1280, has_attn=True)
        self.middleblock = MiddleBlock(n_channels=channels_in+channels_noise, time_channels=1280)
        self.enc_downsample = Downsample(channels_in)
        self.depth_downsample = Downsample(channels_noise)
        self.upsample = Upsample(channels_in+channels_noise)
    ...
    def forward(self, noisy_image, t, *args):
        ...
        noisefuse = torch.cat((feat, noisy_image), dim=1)
        encode = self.downblock(noisefuse, t)
        encodedown = self.enc_downsample(encode, t)
        sparse_depth_down = self.depth_downsample(sparse_depth, t)
        midfusion = torch.cat((encodedown, sparse_depth_down),dim=1)
        midfusion = self.middleblock(midfusion, t)
        midfusion = self.upsample(midfusion, t)
        upfeature = torch.cat((encode, midfusion),dim=1)
        feat = self.upblock(upfeature, t)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, time_channels: int, n_groups: int = 4):
        ...
        * `n_groups` is the number of groups for [group normalization](../../normalization/group_norm/index.html)
        ...
        self.time_emb = nn.Embedding(time_channels, out_channels)
