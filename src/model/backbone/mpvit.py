...
class MPViT(nn.Module):
        ...
        self.patch_embed_stages = nn.ModuleList(
            [
                Patch_Embed_stage(
                    ...
                    isPool=True if idx in pool_stages else False,
                    # isPool=True,
                    norm_cfg=self.conv_norm_cfg,
                )
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            checkpoint = torch.load(pretrained, map_location=lambda storage, loc: storage)['model']
...
def mpvit_tiny(**kwargs):
    ...
    checkpoint = torch.load('pretrained/mpvit_tiny.pth' , map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
...
def mpvit_xsmall(**kwargs):
    ...
    checkpoint = torch.load('pretrained/mpvit_xsmall.pth' , map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
...
def mpvit_small(**kwargs):
    ...
    checkpoint = torch.load('/mnt/cfs/algorithm/yiqun.duan/depth/diffuserdc/src/model/backbone/pretrained/mpvit_small.pth' , map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
...
def mpvit_base(**kwargs):
    ...
    checkpoint = torch.load('/mnt/cfs/algorithm/wenjie.yuan/new/s3depth/pretrain/mpvit_base.pth' , map_location=lambda storage, loc: storage)['model']
    logger = get_root_logger()
    load_state_dict(model, checkpoint, strict=False, logger=logger)
    del checkpoint
    del logger
...
if __name__ == '__main__':
    print("Test MPVIT...")
    patch_size = 1
    h, w =  352//patch_size, 906//patch_size
    device = torch.device('cuda:5')
    in_chans = 3
    img = torch.randn(2, in_chans, h, w).to(device)

    model = mpvit_small(in_chans=in_chans)
    model.to(device)
    print(model)
    ...
    # [torch.Size([2, 128, 176, 353]), torch.Size([2, 216, 88, 177]), torch.Size([2, 288, 44, 89]), torch.Size([2, 288, 22, 45])]