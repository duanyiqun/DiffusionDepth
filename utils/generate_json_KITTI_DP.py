...
parser.add_argument('--name_out', type=str, required=False,
                    default='kitti_dp_alltrain.json', help="Output file name")
...
def generate_json_test():
    ...
    path_base = args.path_root \
                + '/depth_selection/test_depth_prediction_anonymous'
    ...
    for name in list_images:
        path_rgb = 'depth_selection/test_depth_prediction_anonymous/image/' \
                   + name
        path_depth = dummpy_placeholder
        path_gt = dummpy_placeholder
        path_calib = \
            'depth_selection/test_depth_prediction_anonymous/intrinsics/' \
            + name[:-4] + '.txt'
